...
{
    /// <summary>
    /// PulseAudio-based audio deck implementation for Linux.
    /// Uses the same interface as Windows AudioDeck but backed by PulseAudio instead of WASAPI.
    /// </summary>
    public sealed class PulseAudioDeck : IAudioDeck
    {
        ...
        // Thread for PulseAudio playback
        private Thread? _playbackThread;
        private CancellationTokenSource? _playbackCancellation;
        private float[]? _audioBuffer;
        private IntPtr _pulseAudioHandle = IntPtr.Zero;

        public PulseAudioDeck(DeckIdentifier deckId, int deviceNumber = 0)
        {
            if (!PlatformDetection.SupportsLinuxAudio)
            {
                throw new PlatformNotSupportedException(
                    $"PulseAudioDeck requires Linux. Running on: {PlatformDetection.ArchitectureInfo}");
            }
        ...
        /// <summary>
        /// Main playback thread function - reads from audio file and writes to PulseAudio.
        /// </summary>
            ...
            try
            {
                ...
                while (_isPlaying && !_playbackCancellation?.Token.IsCancellationRequested == true)
                {
                    lock (_sync)
                    {
                        ...
                        if (samplesRead > 0)
                        {
                            // In a real implementation, write to PulseAudio here
